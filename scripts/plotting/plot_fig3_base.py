...
# Base models are stored without a leading underscore in the current analysis outputs.
ESTIMATOR = "direct"
...
def display_model(model: str) -> str:
    """Display base model names with the explicit '(b)' suffix."""
    return f"{str(model).lstrip('_')} (b)"
...
def make_figure(
    *,
    ...
    frame = frame.loc[
        np.isfinite(frame["rms_adjustment"])
    ].copy()

    if frame.empty:
        raise RuntimeError(
            f"No finite base-model Direct pair-sample coherence rows "
            f"for probe={probe!r}."
        )

    if (frame["rms_adjustment"] < 0).any():
        raise ValueError(
            "Found negative rms_adjustment values."
        )

    # Validate family names before plotting so an unexpected model name cannot
    # silently receive the wrong color.
    ...
    if y_max is None:
        ...
        n_clipped = int(
            np.sum(
                all_values > plot_y_max
            )
        )

    # Use one shared model ordering for all three datasets. Include the known
    # degenerate base Mistral model even if pair_sample has no rows
    # for it, preserving the complete 12-model layout.
    all_models = sorted(
        set(
            frame["model"].astype(str).unique()
        )
        | {DEGENERATE_MODEL},
        key=model_sort_key,
    )
    ...
    for col_index, dataset in enumerate(DATASETS):
        ...
        for position, model in zip(x, all_models):
            ...
            if model == DEGENERATE_MODEL and len(values) == 0:
                # Preserve the known zero-distance degenerate case with the same
                # horizontal median convention as every other model.
                ax.hlines(
                    0.0,
    ...
    print(
        f"Observed sampled RMS range: "
        f"{all_values.min():.6f} to {observed_max:.6f}"
    )
    print(
        f"Common displayed y-axis: 0 to {plot_y_max:.6f}"
    )
    print(
        f"Base models shown: {len(all_models)}"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plot SI Figure 3 base-model variant as Direct-only "
            "pair-level coherence-distance violins colored by model family."
        )
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Probe shown in the figure. Main-text default: sawmil."
        ),
    )

    parser.add_argument(
        "--stem",
        default="figure_si_3_base",
    )
...
def main() -> None:
    ...
    primary = pair_sample.loc[
        pair_sample["probe"].astype(str).eq(args.probe)
        & pair_sample["estimator"].astype(str).eq(ESTIMATOR)
        & ~pair_sample["model"].astype(str).str.startswith("_")
    ].copy()

    print("=" * 100)
    print("FIGURE 3: DISTANCE TO PROBABILISTIC COHERENCE")
    print("=" * 100)
    print(f"Input:     {path}")
