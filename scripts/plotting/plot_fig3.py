...
def make_figure(
    *,
    ...
    if frame.empty:
        raise RuntimeError(
            f"No finite instruction-tuned Direct pair-sample coherence rows "
            f"for probe={probe!r}."
        )
    ...
    fig.text(
        0.035,
        first_pos.y1 + 0.09,
        "Distance to probabilistic coherence",
        ha="left",
        va="bottom",
        fontsize=7.7,
        fontweight="bold",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plot main-text Figure 3 as instruction-tuned, Direct-only "
