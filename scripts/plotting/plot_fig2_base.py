...
# Exact base-checkpoint set corresponding to the 12 models used in Figure 2.
EXPECTED_MODELS = (
    "llama-3.2-3b",
    "llama-3.1-8b",
    "llama-3.1-70b",
...
def bare_model_name(model: str) -> str:
    """Return the checkpoint name without any instruct-model prefix."""
    return str(model).lstrip("_")


def display_model(model: str) -> str:
    """Display base checkpoint names with the ``(b)`` suffix."""
    return f"{bare_model_name(model)} (b)"


def model_family(model: str) -> str:
    bare = bare_model_name(model).lower()
...
def default_output_stem(
    *,
    ...
    stem = f"figure_si_2_{TOP_MODE_OUTPUT_SUFFIX[top_mode]}_base"
    ...
    *,
    ...
    if top_mode == "relative":
        ...
        missing = [
            ...
            if bare_model_name(model) not in MODEL_DECODER_LAYERS
        ]
        if missing:
            raise ValueError(
        ...
        x_values = np.array(
            [
                MODEL_DECODER_LAYERS[bare_model_name(model)]
                for model in family_panel["model"].astype(str)
            ],
            dtype=float,
        )
...
def make_figure(
    *,
    ...
    if top.empty:
        raise RuntimeError(
            f"No base-model Direct model-summary rows for probe={probe!r}."
        )
    if residual.empty:
        raise RuntimeError(
            f"No base-model Direct cross-model rows for probe={probe!r}."
        )
    ...
    row_space = 1.0 if top_mode == "bars" else 0.82
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plot SI Figure 2 using base Direct models: held-out "
