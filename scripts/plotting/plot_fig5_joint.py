...
# Preferred repository estimator name for Joint-to-Conditional in the
# behavioral outputs. Some cached analyses may use a slightly different label,
# so we accept a small list of exact candidates and choose the one present.
ESTIMATOR_CANDIDATES = ("marginal_complete", "joint", "joint_to_conditional")
...
def resolve_estimator(frame: pd.DataFrame) -> str:
    if "estimator" not in frame.columns:
        raise ValueError("matched_unit_summary.parquet is missing the estimator column.")
    observed = set(frame["estimator"].astype(str).dropna().unique())
    for candidate in ESTIMATOR_CANDIDATES:
        if candidate in observed:
            return candidate
    raise RuntimeError(
        "Could not find a Joint-to-Conditional estimator row in matched_unit_summary.parquet. "
        f"Tried {ESTIMATOR_CANDIDATES}; observed estimators={sorted(observed)}"
    )
...
def prepare_summary(
    ...
    *,
    ...
    if out.empty:
        raise RuntimeError(
            f"No instruction-tuned Joint behavioral-resilience rows remain for probe={probe!r}, sample={sample!r}."
        )
...
def prepare_pair_results(
    ...
    *,
    ...
    if out.empty:
        raise RuntimeError(
            f"No instruction-tuned Joint pair-level behavioral rows remain for probe={probe!r}, sample={sample!r}."
        )
    ...
    *,
    ...
    for dataset_index, dataset in enumerate(DATASETS):
        for model_index, model in enumerate(ordered_models()):
            ...
            if cell.empty:
                raise RuntimeError(
                    f"No pair-level rows for model-level Joint effect {model} / {dataset}."
                )
...
def main() -> None:
    ...
    estimator = resolve_estimator(raw_summary)
    ...
    print("=" * 100)
    print("FIGURE SI 5: BEHAVIORAL RESILIENCE (JOINT)")
    print("=" * 100)
    ...
    print(f"Estimator: Joint-to-Conditional only ({estimator})")
