...
def default_output_stem(
    *,
    ...
    stem = f"figure_2_{TOP_MODE_OUTPUT_SUFFIX[top_mode]}"
...
def make_figure(
    *,
    ...
    fig.text(
        row_title_x,
        top_pos.y1 + 0.065,
        "Stability explained by belief probability",
        ha="left",
        va="bottom",
        fontsize=7.7,
        fontweight="bold",
    ...
    fig.text(
        row_title_x,
        bottom_pos.y1 + (0.045 if bottom_mode == "heatmap" else 0.050),
        "Shared residual structure across models",
        ha="left",
        va="bottom",
        fontsize=7.7,
        fontweight="bold",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plot Figure 2 using instruction-tuned Direct models: held-out "
