...
PROBE_SPECS = (
    ("svm", "SVM probe"),
    ("mean_difference", "Mass Mean probe"),
)
...
def draw_violin(
    ...
    *,
    ...
    if len(values) < 2 or np.unique(values).size < 2:
        ax.hlines(median, position - width * 0.24, position + width * 0.24, color=MEDIAN_COLOR, linewidth=1.05, zorder=6)
        return
    violin = ax.violinplot([values], positions=[position], widths=width, showmeans=False, showmedians=True, showextrema=False, bw_method=bw_method, points=100)
...
def plot_distribution_panel(
    ...
    *,
    ...
    panel = statement_level.loc[statement_level["dataset"].astype(str).eq(dataset)].copy()
    models = sorted(EXPECTED_MODELS, key=model_sort_key)
    x = np.arange(1, len(models) + 1, dtype=float)
    for position, model in zip(x, models):
        model_panel = panel.loc[panel["model"].astype(str).eq(model)]
        values = finite_values(model_panel, "gamma")
        color = FAMILY_COLORS[model_family(model)]
        if model == DEGENERATE_MODEL and dataset in DEGENERATE_DATASETS and len(values) == 0:
            ax.hlines(0.0, float(position) - violin_width * 0.24, float(position) + violin_width * 0.24, color=MEDIAN_COLOR, linewidth=1.05, zorder=6, clip_on=False)
        else:
            draw_violin(ax, values=values, position=float(position), color=color, width=violin_width, alpha=violin_alpha, bw_method=bw_method)
    ax.set_xticks(x)
    ...
    if show_titles:
        ax.set_title(DATASET_NAMES[dataset], loc="center", fontsize=7.2, fontweight="bold", color=TITLE_COLOR, y=1.14, pad=0)


def prepare_family_scale_data(panel: pd.DataFrame, *, family: str, scale_mode: str) -> pd.DataFrame:
    family_panel = panel.loc[panel["model"].astype(str).map(model_family).eq(family)].copy()
    if family_panel.empty:
        return family_panel
    family_panel["params_b"] = pd.to_numeric(family_panel["params_b"], errors="coerce")
    family_panel["mean_gamma"] = pd.to_numeric(family_panel["mean_gamma"], errors="coerce")
    family_panel = family_panel.groupby("model", as_index=False).agg(params_b=("params_b", "mean"), mean_gamma=("mean_gamma", "mean"))
    family_panel = family_panel.loc[np.isfinite(family_panel["params_b"]) & np.isfinite(family_panel["mean_gamma"])].copy()
    if family_panel.empty:
        return family_panel
    family_panel = family_panel.sort_values(["params_b", "model"], kind="stable").reset_index(drop=True)
    if scale_mode == "relative":
        ...
        missing = [display_model(model) for model in family_panel["model"].astype(str) if display_model(model) not in MODEL_DECODER_LAYERS]
        if missing:
            raise ValueError("Missing decoder-layer metadata for models: " + ", ".join(sorted(set(missing))))
        x_values = np.array([MODEL_DECODER_LAYERS[display_model(model)] for model in family_panel["model"].astype(str)], dtype=float)
    else:
...
def configure_scale_axis(ax: plt.Axes, *, scale_mode: str) -> None:
    if scale_mode == "relative":
        ax.set_xlim(-0.15, 2.15)
        ax.set_xticks([0.0, 1.0, 2.0])
        ax.set_xticklabels(["Small", "Medium", "Large"], fontsize=5.8)
    elif scale_mode == "parameters":
...
def plot_scale_panel(
    ...
    *,
    ...
    panel = model_summary.loc[model_summary["dataset"].astype(str).eq(dataset)].copy()
    plotted = 0
    for family in FAMILY_ORDER:
        trajectory = prepare_family_scale_data(panel, family=family, scale_mode=scale_mode)
        if trajectory.empty:
            continue
        ...
        ax.plot(x[valid], y[valid], color=color, linestyle="-", linewidth=1.05, marker="o", markersize=3.5, markerfacecolor=color, markeredgecolor=color, markeredgewidth=0.65, alpha=0.95, zorder=4)
        plotted += 1
    configure_scale_axis(ax, scale_mode=scale_mode)
    style_axis(ax, x_grid=(scale_mode != "relative"))
    if not show_y_labels:
        ax.tick_params(axis="y", labelleft=False)
        ax.spines["left"].set_visible(False)
    if plotted == 0:
        ax.text(0.5, 0.5, "No valid scale groups", transform=ax.transAxes, ha="center", va="center", fontsize=6.0, color="#666666")
...
def make_figure(
    *,
    ...
    statement_by_probe: dict[str, pd.DataFrame] = {}
    for probe, _ in PROBE_SPECS:
        ...
        statement_by_probe[probe] = statements
    ...
    fig.subplots_adjust(left=0.11, right=0.995, top=0.89, bottom=0.16, wspace=0.15, hspace=0.78)

    panel_labels = [f"({chr(ord('a') + i)})" for i in range(6)]
    label_idx = 0

    for probe_idx, (probe, probe_label) in enumerate(PROBE_SPECS):
        statements = statement_by_probe[probe]
    ...
    fig.legend(
        handles=legend_handles, loc="lower center", bbox_to_anchor=(0.55, -0.02),
    ...
    fig.text(
        0.035, first_row_pos.y1 + 0.045, "SVM",
        ha="left", va="bottom", fontsize=7.7, fontweight="bold", color=TITLE_COLOR,
    )
    fig.text(
        0.035, second_row_pos.y1 + 0.045, "Mass Mean",
        ha="left", va="bottom", fontsize=7.7, fontweight="bold", color=TITLE_COLOR,
    )
    fig.text(
        0.04, (first_row_pos.y0 + first_row_pos.y1) / 2.0,
        r"Graded stability $\gamma(P)$", ha="center", va="center", rotation=90, fontsize=7,
    )
    fig.text(
        0.04, (second_row_pos.y0 + second_row_pos.y1) / 2.0,
        r"Graded stability $\gamma(P)$", ha="center", va="center", rotation=90, fontsize=7,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot SI Figure 4 graded-stability distributions for SVM and Mass Mean probes."
    )
    ...
    parser.add_argument(
        "--stem", default="figure_si_4_relative_scale_svm_mass_mean",
...
def main() -> None:
    ...
    print("=" * 100)
    print("FIGURE SI 4: GRADED STABILITY DISTRIBUTIONS (SVM + MASS MEAN)")
    print("=" * 100)
