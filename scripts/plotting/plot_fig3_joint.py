...
def make_figure(
    *,
    ...
    if frame.empty:
        raise RuntimeError(
            f"No finite instruction-tuned Joint-to-Conditional pair-sample coherence rows "
            f"for probe={probe!r}."
        )
    ...
    # degenerate instruction-tuned Mistral model even if pair_sample has no rows
    ...
    print(
        f"Instruction-tuned models shown: {len(all_models)}"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Plot SI Figure 3 as instruction-tuned, Joint-to-Conditional-only "
...
def main() -> None:
    ...
    print("Estimator: Joint-to-Conditional only")
    print()
