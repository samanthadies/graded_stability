...
def make_figure(
    *,
    ...
    if statements.empty:
        raise RuntimeError(
            f"No instruction-tuned Joint statement-level rows for probe={probe!r}."
        )
    ...
    missing_statement_models = [
        model for model in EXPECTED_MODELS if model not in present_statement_models
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot instruction-tuned Joint graded-stability distributions."
    )
    parser.add_argument(
        "--input_dir",
        type=Path,
    ...
    parser.add_argument(
        "--stem",
        default="figure_si_4_relative_scale_joint",
        help=(
            "Output stem. The default preserves the filename already referenced "
            "by the SI even though this revised figure contains only the top row."
        ),
    )
...
def main() -> None:
    ...
    print("=" * 100)
    print("FIGURE SI 4: GRADED STABILITY DISTRIBUTIONS (JOINT)")
    print("=" * 100)
