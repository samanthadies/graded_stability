...
from stability.beliefs.conditional import (
    ConditionalTrainingData,
    build_conditional_training_data,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train direct-conditional probes and stream-score compact pair "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "configs/experiments/conditional.yaml"
        ),
    )
    parser.add_argument(
        "--probes",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Conditional rendering variant from configs/statements.yaml. "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional per-probe test-pair limit for smoke tests."
        ),
    )
    ...
    *,
    ...
    try:
        ...
        if not np.allclose(
            ...
            raise RuntimeError(
                f"{probe_name}: output probabilities do not sum to one."
            )
...
def main() -> None:
    ...
    train_per_pair_type = int(
        _setting(
            args.train_per_pair_type,
            experiment,
            "train_per_pair_type",
            750,
        )
    )
    cal_per_pair_type = int(
        _setting(
            args.cal_per_pair_type,
            experiment,
            "cal_per_pair_type",
            250,
        )
    ...
    if pair_probe_frame.empty:
        ...
        if (
            ...
            raise FileExistsError(
                "Conditional outputs already exist. "
        ...
        if args.resume and summary_path.exists():
            ...
            if (
                ...
                print(
                    f"Conditional output already complete "
        ...
        skipped_probes = {
            probe_name: {
                ...
                "reason": "no_pair_rows",
                "message": (
                    "No P x x pair rows exist for this probe, so conditional "
        ...
        print("=" * 72)
        print("DIRECT CONDITIONAL PHASE")
        print("=" * 72)
    ...
    if (
        ...
        raise FileExistsError(
            "Conditional outputs already exist. Use --resume or --overwrite."
        )
    ...
    if output_path.exists() and args.resume:
        ...
        if summary_path.exists():
            with summary_path.open(
                ...
                existing_summary = json.load(
                    handle
                )
            known_skipped.update(
        ...
        for probe in probes:
            if probe in known_skipped:
                continue
            expected = _pair_count(
                pair_path,
                probe_name=probe,
            )
        ...
        if complete:
            print(
                f"Conditional output already complete: {output_path}"
            )
            return

        raise RuntimeError(
            "A final conditional Parquet exists but is incomplete. "
            "Use --overwrite to rebuild; partial resumable state should live "
            "in the hidden parts directory, not the final Parquet."
        )

    conditional_train = build_conditional_training_data(
        data,
        train_per_pair_type=train_per_pair_type,
        cal_per_pair_type=cal_per_pair_type,
        seed=seed,
        semantics=semantics,
        template=str(
            template
    ...
    if bundle is None:
        ...
        if mismatches:
            raise ValueError(
                f"Existing conditional probe bundle does not match this run: "
                f"{mismatches}"
            )
    ...
    print(
        "Conditional train/cal: "
        f"{conditional_train.train_mask.sum():,} / "
        f"{conditional_train.cal_mask.sum():,}"
    )
    print(f"Pair table:        {pair_path}")
    ...
    try:
        ...
        if missing_probe_models:
            ...
            print("\nTraining new direct-conditional probes...")

            for layer, layer_probes in grouped.items():
                print(
                    f"\nConditional training activations: "
                ...
                print(
                    f"Collected training activations: "
                ...
                try:
                    for probe_name in layer_probes:
                        print(
                            f"[{probe_name}] fitting separate conditional probe..."
                        )
                        ...
                        try:
                            probe.fit(
                                activations,
                                conditional_train.labels,
                                conditional_train.train_mask,
                                conditional_train.cal_mask,
                            )
                        ...
                        train_metrics = _evaluate_split(
                            ...
                            labels=conditional_train.labels,
                            mask=conditional_train.train_mask,
                        )
                        cal_metrics = _evaluate_split(
                            probe=probe,
                            activations=activations,
                            labels=conditional_train.labels,
                            mask=conditional_train.cal_mask,
                        )
            ...
            print(
                "\nAll requested direct-conditional probe models already "
                "exist in the bundle; skipping training."
            )
        ...
        if not active_probes:
            raise RuntimeError(
                "No requested probe could be trained for conditional scoring."
            )
        ...
        for probe_name in active_probes:
            ...
            if completed:
                print(
                    f"\n[{probe_name}] resuming at pair row "
                    f"{completed:,}/{len(pairs):,}."
                )
            else:
                print(
                    f"\n[{probe_name}] scoring {len(pairs):,} conditional pairs "
                    f"at layer {layer}."
                )
        ...
        if total_rows != expected_total:
            raise RuntimeError(
                f"Consolidated {total_rows:,} conditional score rows; "
        ...
        summary = {
            ...
            "training_data": conditional_train.summary(),
        ...
        print("\n" + "=" * 72)
        print("DIRECT CONDITIONAL SCORING COMPLETE")
        print("=" * 72)
        ...
        for probe_name in active_probes:
            ...
            print(
                ...
                f"pairs={info['rows']:,} | "
                f"conditional-cal log-loss={metrics['log_loss']:.6f}"
            )
