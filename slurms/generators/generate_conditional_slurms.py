...
Phase 2:
  * generates balanced Cooper-Cantwell conditional train/cal examples in memory;
  * trains NEW direct-conditional probes at the probe-specific selected layers;
  * saves those fitted probes in outputs/conditional/<model>/<dataset>.joblib;
  * streams P x x test pairs without saving rendered statements/activations;
  * checkpoints hidden Parquet parts and resumes from completed chunks.

Generated files:
    slurms/generated/conditional/<model>__<dataset>.slurm

Logs:
    slurms/generated/conditional/logs/

Required upstream input:
    outputs/pairs/<model>/<dataset>.parquet

Canonical outputs:
    outputs/conditional/<model>/<dataset>.parquet
    outputs/conditional/<model>/<dataset>.joblib
    outputs/conditional/<model>/<dataset>.json
"""
...
def hours_for_size(
    ...
    """
    Phase 2 is dominated by streaming millions of conditional test pairs.

    The defaults deliberately use a 24-hour checkpoint window for most models.
    Because score_conditionals uses --resume and persists both trained probes
...
def render_job(
    *,
    ...
    job_name = (
        f"cond-{model_token}-{dataset_token}"
    )
    ...
    lines = [
        ...
        "",
        'echo "============================================================"',
        'echo "Graded Stability: Phase 2 direct-conditional scoring"',
        ...
        "fi",
        "",
        "# --resume preserves fitted conditional probes and scored checkpoint parts.",
        "srun python -m scripts.beliefs.score_conditionals \\",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the full 72-job GPU Slurm matrix for Phase 2 "
            "direct-conditional scoring."
        )
    )
...
def main() -> None:
    ...
    print(
        f"Generated {len(generated)} Phase-2 Slurm files "
