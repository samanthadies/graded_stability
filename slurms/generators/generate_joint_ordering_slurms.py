...
"""
Generate the full 24-model x 3-dataset Slurm matrix for the Joint-probe
statement-ordering sensitivity analysis.

Each generated GPU job runs:

    python -m scripts.beliefs.score_joint

for one model x one dataset with --template P_then_x and --resume.

Ordering sensitivity run:
  * uses the SAME probe-specific P x x pair tables as the main experiment;
  ...
  * renders Joint statements as "P and x" instead of the main "x and P";
  * saves all new results under outputs/joint_ordering/P_then_x/ so the
    canonical outputs/joint/ results are never touched;
  * checkpoints hidden Parquet parts and resumes from completed chunks.

Generated files:
    slurms/generated/joint_ordering/P_then_x/<model>__<dataset>.slurm

Logs:
    slurms/generated/joint_ordering/P_then_x/logs/

Required upstream input:
    outputs/pairs/<model>/<dataset>.parquet

Ordering-sensitivity outputs:
    outputs/joint_ordering/P_then_x/<model>/<dataset>.parquet
    outputs/joint_ordering/P_then_x/<model>/<dataset>.joblib
    outputs/joint_ordering/P_then_x/<model>/<dataset>.json
"""
...
def render_job(
    *,
    ...
    job_name = (
        f"joint-order-{model_token}-{dataset_token}"
    )
    ...
    lines = [
        ...
        f'echo "Joint template: {ORDERING_TEMPLATE}"',
        ...
        f"  --dataset {q(dataset)} \\",
        f"  --template {q(ORDERING_TEMPLATE)} \\",
        f"  --output_dir {q(output_root)} \\",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the full 72-job GPU Slurm matrix for the Joint-probe "
            "P_then_x statement-ordering sensitivity analysis."
        )
    )
...
def main() -> None:
    ...
    print(
        f"Generated {len(generated)} Joint-ordering Slurm files "
